...
### Thumbnail rendering (UsdThumbnail.py)

Thumbnails are generated by `UsdPythonToolsLocalServer.exe`, which extracts `UsdThumbnail.py` from its resources to `%TEMP%` and launches it as a Python subprocess via `CreateProcess`. The script proceeds in four steps:

1. **PXR_PLUGINPATH_NAME narrowing.** The COM parent sets `PXR_PLUGINPATH_NAME` to the install directory, which includes the `UsdShellExtension.dll` ArResolver plugin. Loading that DLL in the subprocess corrupts `UsdImagingGL.Engine`. Before any pxr import, the script redirects `PXR_PLUGINPATH_NAME` to the SDK's `plugin/usd/` directory (derived from the first `lib/python` entry in `PYTHONPATH`). This keeps HdStorm and MaterialX available without loading `UsdShellExtension.dll`.

2. **Offscreen OpenGL context.** A `QOffscreenSurface` + `QOpenGLContext` + 1×1 `QOpenGLFramebufferObject` is created before importing `UsdImagingGL`. This gives HdStorm a valid GL context without opening any window.

3. **Camera selection.** If the stage has an authored camera, `_find_stage_camera()` returns it (primary camera name first, then any `UsdGeom.Camera`). Otherwise `_make_framing_camera()` writes a square (1:1, 36×36 mm aperture, 45° FOV) perspective camera to the session layer. The camera is front-facing: placed on −Y for Z-up scenes, +Z for Y-up scenes, and oriented with the GLM-style `Cross(forward, up)` formula to avoid horizontal mirroring.

4. **Render.** `UsdAppUtils.FrameRecorder` renders at `startTimeCode`, 256×256 px, sRGB colour correction, proxy purpose.

### Preview pane context menu

The right-click context menu in the preview pane rebuilds on every `contextMenuEvent` call (`buildContextMenu()` is called before `exec()`). This ensures `GetRendererPlugins()` is queried after the Hydra engine has initialized, keeping the Hydra Renderer submenu active.
